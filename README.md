...
이미지 URL 셀에는 여러 주소를 줄바꿈, 쉼표, 세미콜론 또는 `|`로 구분해 입력할 수 있습니다. 상세 팝업은 입력 순서대로 이미지를 보여주며 좌우 화살표와 키보드 방향키로 이동합니다. `ImageURLs`, `ImageURL2`, `ImageURL3` 열을 추가하는 방식도 지원합니다.
...
## 기능
패턴 상세 분류와 30개 페이지, 작은 이미지와 우측 이름 카드, 효산 LPM 전체/솔리드/우드/스톤/패브릭 필터(시트 추가 분류 포함), 상세 정보와 관련 항목 이동, 경면판 추천, 다중 이미지 갤러리, 이미지 150% 확대, X/외부 클릭/Escape 닫기를 유지했습니다.
